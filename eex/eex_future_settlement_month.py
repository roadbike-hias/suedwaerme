...
start_date = datetime.strptime('24.12.2023', '%d.%m.%Y')
finish_date = datetime.strptime('19.03.2025', '%d.%m.%Y')
...
for i in range(iterations):
    ...
    if response.status_code == 200:
        ...
        for item in data['results']['items']:
            ...
            month_char = pricesymbol[-3]  # Get the month character (J, N, V, F)
            year = pricesymbol[-2:]  # Get the last two digits (year)
            month = month_mapping.get(month_char, 'Unknown')  # Map to month
            reformatted_pricesymbol = f"{month} 20{year}"  # Format as "Qx yyyy"
...
with open('eex_future_settlement_month.json', 'w') as file:
    json.dump(all_results, file, indent=4)

# Write reformatted data to a CSV file
with open('eex_future_settlement_month.csv', 'w', newline='') as file:
    writer = csv.writer(file)
    # Write header
    writer.writerow(['Month', 'Trade Date', 'Close'])
    # Write data
...
def custom_sort_key(column):
    """Sort columns by year and then by month (Q1, Q2, Q3, Q4)."""
    month, year = column.split()
    year = int(year)
    month_order = {'Q1': 0, 'Q2': 1, 'Q3': 2, 'Q4': 3}
    return (year, month_order.get(month, 4))
...
df.to_excel('eex_future_settlement_month.xlsx')

print("Data has been written to eex_future_settlement_month.json, eex_future_settlement_month.csv, and eex_future_settlement_month.xlsx")